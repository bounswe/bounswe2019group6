...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.createdAt >= ?1 AND t.createdAt <= ?2 AND t.user.username= ?3")
    List<Transaction> findByDateBetweenOfUser(Date start, Date end, String username);
